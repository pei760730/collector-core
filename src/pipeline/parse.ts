...
//    左右必須對稱:原本只列了右括號家族,而 NON_URL_CHAR 又把 ( [ ~ * + 當成合法 URL
//    字元、不在那裡截斷 —— 於是 `<url>(說明` 會產出一個結尾帶 `(` 的 rawUrl,原封不動
//    存進「連結」欄(人點下去 404),抽不出 id 的平台還會因為 path fallback 保留那個雜
//    字元而變成兩把去重鍵、兩列。支援清單裡沒有任何真實平台網址以這些字元結尾。
