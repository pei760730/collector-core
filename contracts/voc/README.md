...
> 這裡是 **vendored 副本**:collector-core 是 TS pipeline 的 SSOT,必須**自己**對這份契約跑 conformance(`tests/dedupConformance.test.ts`),不能只靠下游 collector(short-video-bot 等)在 bump dep 後才測出漂移。

## 為什麼 vendor

voc 是 private,collector-core 無法在 CI 跨 repo 抓檔。沿用 short-video-bot 既有做法(它把同一份 vendored 到自己的 `contracts/voc/`)。
